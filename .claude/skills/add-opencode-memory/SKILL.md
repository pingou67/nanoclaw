...
description: Add persistent Claude Code-compatible memory to opencode-backed agent groups via the opencode-claude-memory plugin (local shim, no runtime npm install, works behind the OneCLI proxy; requires /add-opencode).
---
...
Canonical payload: `resources/opencode-claude-memory.js` (mirror of
`container/opencode-plugins/opencode-claude-memory.js`), refreshed with
`pnpm exec tsx scripts/skills-sync.ts sync add-opencode-memory`. Manifest:
`skill-sync.json`.

> **Variante MCP retirée le 2026-08-11.** Le skill embarquait aussi un serveur
> MCP stdio (`mcp-servers/memory/server.ts`) exposant les mêmes outils
> `memory_*` aux providers non-opencode. La migration vers l'arbre mémoire OKF
> (2026-07-19) l'a rendu inutile : plus aucun groupe ne le déclarait, et
> l'injection de contexte mémoire passe désormais par le runner, pour tous les
> providers. Récupérable dans l'historique git si le besoin revenait.
