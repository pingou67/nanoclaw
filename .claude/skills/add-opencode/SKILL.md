...
`summarize.ts` formats one-line tool-call summaries for the live-status feature; it ships with this skill because the opencode provider consumes it (the fork's claude provider imports it too — see the fork note at the end). The three extra `opencode.*.test.ts` files guard the fork's local provider fixes (per-query SSE, plugins, tool-progress).
