...
- **Per-provider wiring**:
  ...
  - **agy** — Antigravity DOES have JSON hooks (`hooks.json` in `.agents/` or `~/.gemini/config/`, schema: map hookName → `{PreToolUse: [{matcher, hooks: [...]}]}`, see antigravity.google/docs/hooks), but its `PreToolUse` output contract is **decision-only** (`allow`/`deny`/`ask` + `permissionOverrides`) — no tool-input rewrite. So interception is done at the **shell level**: `container/rtk-shims/` (wrappers for git, gh, docker, kubectl, cargo, npm, pnpm, npx, tsc, jest, vitest, psql, aws) mounted RO at `/opt/rtk-shims`, and the agy provider (`container/agent-runner/src/providers/agy.ts`) prepends it to PATH **for the agy process tree only**. Each shim execs `rtk <cmd>` with an env recursion guard (`RTK_SHIM_GUARD`). Deliberately NOT shimmed: `curl`, `grep`, `ls`, `find`, `rg`, `wget` — too pervasive in scripts/MCP and rtk's transforms would corrupt programmatic consumers.
  - **codex** — no group yet; same PATH-shim approach would apply (prepend in the codex provider spawn).
