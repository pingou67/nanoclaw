...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  for (const agent of agents) {
    ...
    if (engages && accessOk && scopeOk) {
      await safeDeliverToAgent(agent, agentGroup, mg, event, userId, threadsEnabled, effectiveThreadId, true);
      engagedCount++;
      ...
      await safeDeliverToAgent(agent, agentGroup, mg, event, userId, threadsEnabled, effectiveThreadId, false);
      accumulatedCount++;
    } else {
...
async function safeDeliverToAgent(
  ...
  try {
    await deliverToAgent(agent, agentGroup, mg, event, userId, threadsEnabled, effectiveThreadId, wake);
  } catch (err) {
