  ...
  const bridge: ChannelAdapter = {
    ...
    async deliver(platformId: string, threadId: string | null, message): Promise<string | undefined> {
      ...
      if (content.operation === 'edit' && content.messageId) {
        // Structure upstream (carte d'approbation terminale) + notre
        // `unwrapMessageId`, appliqué aux DEUX branches : sans lui, l'édition
        // vise un identifiant encore encapsulé et Mattermost ne trouve pas le
        // message.
        ...
        if (
          ...
          await adapter.editMessage(
            tid,
            unwrapMessageId(content.messageId as string),
