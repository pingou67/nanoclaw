  ...
  customOperations: {
    ...
    'config update': {
      ...
      description:
        ...
        '--thinking, --thinking-budget-tokens (thinking values: adaptive — Claude decides, enabled — fixed budget paired with --thinking-budget-tokens N, ' +
      ...
      handler: async (args) => {
        ...
        if (Object.keys(updates).length === 0 && thinkingUpdate === undefined) {
          throw new Error(
            'Nothing to update — provide at least one of: --provider, --model, --effort, --image-tag, --assistant-name, --max-messages-per-prompt, --cli-scope, --thinking, --timezone',
          );
        }

        if (Object.keys(updates).length > 0) await updateContainerConfigScalars(id, updates);
        if (thinkingUpdate !== undefined) await updateContainerConfigJson(id, 'thinking', thinkingUpdate);
