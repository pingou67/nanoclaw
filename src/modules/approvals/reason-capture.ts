...
export async function armReasonCapture(approval: PendingApproval, session: Session, userId: string): Promise<void> {
  ...
  if (displaced && displaced.approvalId !== approval.approval_id) {
    ...
    const prior = await getPendingApproval(displaced.approvalId);
    if (prior && prior.status === 'awaiting_reason') {
      const priorSession = prior.session_id ? await getSession(prior.session_id) : null;
      if (priorSession) {
        ...
        await deletePendingApproval(prior.approval_id);
      }
    }
