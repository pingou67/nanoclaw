...
registerGatewayProvider('onecli', () => ({
  ...
  async contribute({ key, groupName }) {
    ...
    //
    // Ecart fork : le serveur OneCLI v1 valide strictement les identifiants
    // (minuscules, chiffres et TIRETS - pas d'underscore), alors que nos
    // agent_group.id en contiennent (`ag-mattermost_dm`). Nos agents OneCLI
    // existent deja sous la forme a tirets (`ag-mattermost-dm`), avec leur mode
    // `selective` et leurs secrets assignes. Sans cette traduction au point
    // d'appel, ensureAgent viserait un identifiant inconnu : la passerelle
    // recreerait des agents neufs, en mode `all`, et le perimetre de secrets
    // patiemment regle serait perdu sans le moindre message. L'id interne de
    // nanoclaw est inchange - FK et chemins de fichiers continuent de marcher.
    const agentIdentifier = key.agentGroupId.replace(/_/g, '-');
