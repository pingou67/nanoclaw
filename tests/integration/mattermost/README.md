...
| **MCP matrix** | One scenario per MCP server wired in `container_configs` (vikunja, imap, gmail, google-calendar, searxng…). Each picks the first E2E-reachable channel whose group has the server (optionally preferring the group whose credentials matter most, e.g. famille's own calendar OAuth), sends a **read-only** prompt forcing one MCP call, and asserts a stable backend invariant (`WORK` project, `INBOX` folder/label, calendar name, `Paris`). A server wired on no reachable group (e.g. `memory` on the agy-backed testor-agy) is a SKIP. Catches MCP config drift (DB `mcp_servers`, mounts, HOME overrides, OAuth tokens) that unit tests can't see. Run just this phase with `--only-mcp` (~3-4 min). |
