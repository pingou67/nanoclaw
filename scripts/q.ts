...
if (path.resolve(dbPath) === path.resolve(CENTRAL_DB_PATH)) {
  ...
  // fileMustExist : sans lui, better-sqlite3 CRÉE une base vide sur un chemin
  // mal tapé et la requête « réussit » sur zéro ligne — l'erreur réelle est
  // masquée. Ne concerne que les chemins de session explicites : la base
  // centrale passe par initDb au-dessus.
  if (!fs.existsSync(dbPath)) {
    console.error(`Error: database file not found: ${dbPath}`);
    process.exit(2);
  }
  db = new SqliteDriver(new Database(dbPath, { fileMustExist: true }));
  close = () => db.close();
}
try {
  if (isQuery(sql)) {
