...
import { makeActiveQuery } from '../poll-loop.test-support.js';
...
async function runThroughPollLoop(id: string): Promise<{ delivered: string[]; pushes: string[] }> {
  ...
  await processQuery(makeActiveQuery(query, CHAT_ROUTING, ['m1'], 'prompt', undefined), 'claude', undefined, true);
...
// Le live-status est une fonctionnalité du fork : elle écrit une ligne de chat
// supplémentaire (« 🔧 Bash — 1 action »), que les attentes d'upstream ignorent.
// Ces scénarios mesurent la LIVRAISON, pas l'affichage de progression, donc on
// le neutralise ici plutôt que de filtrer les lignes après coup.
process.env.NANOCLAW_LIVE_STATUS_DISABLED = '1';
