 ...
 * ni au provider.
 *
 * Auth : variables d'env `VIKUNJA_URL` (ex. https://vikunja.pegs.fr) et
 * `VIKUNJA_TOKEN` (token d'API Vikunja). Rien n'est jamais loggé sur stdout
 * (réservé au protocole MCP) — les diagnostics vont sur stderr.
 *
...
/** Compose le corps d'une tâche à partir d'un objet d'arguments (champs optionnels). */
...
type Task = { id: number; title: string; done?: boolean; due_date?: string; priority?: number; project_id?: number; percent_done?: number };
...
const server = new McpServer({ name: 'vikunja', version: '1.0.0' });
...
reg(
  'list_tasks',
  "Liste les tâches de l'utilisateur (tous projets), avec filtre/recherche/tri. Par défaut : non terminées, triées par échéance. Sortie lisible.",
  {
    ...
    include_done: z.boolean().optional().describe('Si true, ne pas filtrer les terminées (ignore le filtre done par défaut).'),
    raw: z.boolean().optional().describe('Si true, renvoie le JSON brut au lieu de la liste formatée.'),
  },
  async (a) => {
    const filter = a.filter ?? (a.include_done ? undefined : 'done = false');
    const tasks = (await api('GET', '/tasks', undefined, {
    ...
    if (!Array.isArray(tasks) || tasks.length === 0) return '(aucune tâche)';
    return tasks.map(fmtTask).join('\n');
  },
);

reg('get_task', "Détail complet d'une tâche (JSON).", { id: z.number() }, (a) => api('GET', `/tasks/${a.id}`));
...
reg(
  ...
  (a) => api('POST', `/tasks/${a.id}`, taskBody(a)),
);

reg('complete_task', 'Marque une tâche comme terminée (raccourci de update_task done=true).', { id: z.number() }, (a) =>
  api('POST', `/tasks/${a.id}`, { done: true }),
);
...
reg(
  ...
  (a) => api('POST', '/tasks/bulk', { task_ids: a.task_ids, ...(a.fields as object) }),
);

reg('duplicate_task', 'Duplique une tâche dans un projet cible.', { id: z.number(), project_id: z.number() }, (a) =>
  api('PUT', `/tasks/${a.id}/duplicate`, { project_id: a.project_id }),
);

// ─────────────────────────── Comments ───────────────────────────
reg('list_comments', "Liste les commentaires d'une tâche.", { task_id: z.number() }, (a) =>
  api('GET', `/tasks/${a.task_id}/comments`),
);
reg('add_comment', 'Ajoute un commentaire à une tâche.', { task_id: z.number(), comment: z.string() }, (a) =>
  api('PUT', `/tasks/${a.task_id}/comments`, { comment: a.comment }),
);
reg('update_comment', 'Modifie un commentaire.', { task_id: z.number(), comment_id: z.number(), comment: z.string() }, (a) =>
  api('POST', `/tasks/${a.task_id}/comments/${a.comment_id}`, { comment: a.comment }),
);
...
reg(
  ...
  (a) => api('PUT', `/tasks/${a.task_id}/relations`, { other_task_id: a.other_task_id, relation_kind: a.relation_kind }),
);
...
// ─────────────────────────── Labels ───────────────────────────
reg('list_labels', "Liste tous les labels de l'utilisateur.", {}, () => api('GET', '/labels'));
reg('create_label', 'Crée un label.', { title: z.string(), description: z.string().optional(), hex_color: z.string().optional() }, (a) =>
...
reg('list_task_labels', "Liste les labels posés sur une tâche.", { task_id: z.number() }, (a) => api('GET', `/tasks/${a.task_id}/labels`));
reg('add_label_to_task', 'Ajoute un label à une tâche.', { task_id: z.number(), label_id: z.number() }, (a) =>
  api('PUT', `/tasks/${a.task_id}/labels`, { label_id: a.label_id }),
);
...
reg('set_task_labels', "Remplace l'ensemble des labels d'une tâche.", { task_id: z.number(), label_ids: z.array(z.number()) }, (a) =>
  api('POST', `/tasks/${a.task_id}/labels/bulk`, { labels: (a.label_ids as number[]).map((id) => ({ id })) }),
);

// ─────────────────────────── Assignees ───────────────────────────
// L'endpoint dédié GET /tasks/{id}/assignees renvoie un 500 sur Vikunja v2.3.0 ;
// on lit le champ `assignees` de la tâche, fiable.
reg('list_assignees', "Liste les personnes assignées à une tâche.", { task_id: z.number() }, async (a) => {
  const t = (await api('GET', `/tasks/${a.task_id}`)) as { assignees?: unknown[] };
  return t.assignees ?? [];
});
reg('add_assignee', 'Assigne un utilisateur à une tâche.', { task_id: z.number(), user_id: z.number() }, (a) =>
  api('PUT', `/tasks/${a.task_id}/assignees`, { user_id: a.user_id }),
);
...
// ─────────────────────────── Projects ───────────────────────────
reg('list_projects', "Liste les projets/listes accessibles (id + titre).", { raw: z.boolean().optional() }, async (a) => {
  const ps = (await api('GET', '/projects')) as Array<{ id: number; title: string; is_archived?: boolean }>;
  if (a.raw) return ps;
  return ps.map((p) => `${p.id}\t${p.title}${p.is_archived ? ' (archivé)' : ''}`).join('\n');
});
reg('get_project', "Détail d'un projet.", { id: z.number() }, (a) => api('GET', `/projects/${a.id}`));
reg('create_project', 'Crée un projet/liste.', { title: z.string(), description: z.string().optional(), parent_project_id: z.number().optional(), hex_color: z.string().optional() }, (a) =>
  api('PUT', '/projects', { title: a.title, description: a.description, parent_project_id: a.parent_project_id, hex_color: a.hex_color }),
);
...
reg('duplicate_project', 'Duplique un projet.', { id: z.number(), parent_project_id: z.number().optional() }, (a) =>
  api('PUT', `/projects/${a.id}/duplicate`, { parent_project_id: a.parent_project_id ?? 0 }),
);

// ─────────────────────────── Saved filters ───────────────────────────
reg('create_filter', 'Crée un filtre sauvegardé.', { title: z.string(), filter: z.string().describe('Requête de filtre, ex: priority >= 4 && done = false'), description: z.string().optional() }, (a) =>
...
reg('update_filter', 'Modifie un filtre sauvegardé.', { id: z.number(), title: z.string().optional(), filter: z.string().optional(), description: z.string().optional() }, async (a) => {
  ...
  return api('POST', `/filters/${a.id}`, {
    title: a.title ?? cur.title,
    description: a.description ?? cur.description,
    filters: a.filter !== undefined ? { filter: a.filter } : cur.filters,
  });
});
...
reg('subscribe', "S'abonne à une entité (entity: task|project). entity_id = son id.", { entity: z.enum(['task', 'project']), entity_id: z.number() }, (a) =>
  api('PUT', `/subscriptions/${a.entity}/${a.entity_id}`, {}),
);
...
reg('list_attachments', "Liste les pièces jointes d'une tâche (métadonnées ; l'upload/download binaire n'est pas exposé).", { task_id: z.number() }, (a) =>
  api('GET', `/tasks/${a.task_id}/attachments`),
);
reg('get_current_user', "Infos de l'utilisateur courant (id, username, timezone…).", {}, () => api('GET', '/user'));

const transport = new StdioServerTransport();
await server.connect(transport);
console.error('[vikunja-mcp] prêt (' + BASE + ')');
