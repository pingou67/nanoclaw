...
export function sqliteReleaseMessages(ids: string[]): void {
  if (ids.length === 0) return;
  const db = getOutboundDb();
  const statement = db.prepare('DELETE FROM processing_ack WHERE message_id = ?');
  db.transaction(() => {
...
export function sqliteGetDeliveredPlatformId(outboundMessageId: string): string | null {
  const inbound = openInboundDb();
  try {
    const row = inbound
      .prepare('SELECT platform_message_id FROM delivered WHERE message_out_id = ?')
      .get(outboundMessageId) as { platform_message_id: string | null } | undefined;
