...
export function buildCompactInstructions(names: string[], taskId: string | null): string {
  const deliveryReminder = taskId
    ...
    : [
        // Fork §31 (structuredDelivery) : ce script n'est invoqué que par le
        // hook PreCompact de Claude Code — la branche chat suit donc le contrat
        // structured (réponse directe, jamais d'enveloppe <message>).
